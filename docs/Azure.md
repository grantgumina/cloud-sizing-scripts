...
2. Install required Azure PowerShell modules (plus `PwshSpectreConsole` for the polished console UI):
   ...
   install (rather than silently skipping SQL / Backup / AVS). `PwshSpectreConsole` is required for interactive
   runs; headless / automation runs started with `-NonInteractive` fall back to plain, pipe-safe text and do not need it.
