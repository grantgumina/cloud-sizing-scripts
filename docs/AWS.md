...
2. Enter PowerShell:
   ...
   pwsh
   ```
3. (Install ImportExcel + PwshSpectreConsole in CloudShell)
   ```powershell
   Install-Module -Name ImportExcel,PwshSpectreConsole -Scope CurrentUser -Force
   ```
   ...
   > **Getting the scripts into CloudShell:** clone the repo — `git clone <repo-url>` then `cd dy-cloud-sizing-scripts/src`.
   > The scripts load their shared console layer from `common/` next to them, so run them from `src/`.
